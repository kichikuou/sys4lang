  ...
  let no =
    ...
    | Some existing_g ->
        ...
        if not (Variable.equal g.variable existing_g) then
          link_error
            (Printf.sprintf "Global declaration mismatch: %s" g.variable.name);
        existing_g.index
    | None ->
...
let link_function reloc ain (f : Function.t) =
  let no =
    ...
    | Some existing_f ->
        ...
        if Function.is_defined f then (
          (* ensure function is only defined once *)
          if Function.is_defined existing_f then
            link_error
              (Printf.sprintf "Multiple definitions for function: %s" f.name);
    ...
    | None ->
        ...
        if String.equal f.name "main" then set_main_function ain no
        else if String.equal f.name "message" then set_message_function ain no;
        no
  in
  (* add relocation to table *)
...
let rec link_type reloc ain (v : Type.t) =
  match v.data with
  | Struct no -> (
      match Hashtbl.find reloc.structs no with
      | Some reloc_no -> { v with data = Struct reloc_no }
      | None -> linker_bug (Printf.sprintf "struct type not found: %d" no))
  | FuncType no -> (
      match Hashtbl.find reloc.functypes no with
      | Some reloc_no -> { v with data = FuncType reloc_no }
      | None -> linker_bug (Printf.sprintf "functype not found: %d" no))
  | Delegate no -> (
      match Hashtbl.find reloc.delegates no with
      | Some reloc_no -> { v with data = Delegate reloc_no }
      | None -> linker_bug (Printf.sprintf "delegate not found: %d" no))
  | Array t -> { v with data = Array (link_type reloc ain t) }
  | Wrap t -> { v with data = Wrap (link_type reloc ain t) }
  | Option t -> { v with data = Option (link_type reloc ain t) }
  | Unknown87 t -> { v with data = Unknown87 (link_type reloc ain t) }
...
let link_struct_ctor_dtor ain func_reloc (s : Struct.t) =
  (if s.constructor >= 0 then
     match Hashtbl.find func_reloc s.constructor with
     | Some i -> write_struct ain { s with constructor = i }
     | None ->
         linker_bug
           (Printf.sprintf "constructor declaration not found: %s" s.name));
  if s.destructor >= 0 then
    match Hashtbl.find func_reloc s.destructor with
    | Some i -> write_struct ain { s with destructor = i }
    | None ->
        linker_bug
          (Printf.sprintf "destructor declaration not found: %s" s.name)
  ...
  let no =
    ...
    | Some existing_f ->
        ...
        if not (FunctionType.equal f existing_f) then
          link_error
            (Printf.sprintf "Function type declaration mismatch: %s" f.name);
        existing_f.index
    | None ->
...
let link_code reloc a b =
  ...
  let instruction_iter dasm =
    ...
    if phys_equal opcode 0x61 then
...
let link a b decl_only =
  ...
  if (not decl_only) && code_size b > 0 then link_code reloc a b
