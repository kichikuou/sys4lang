...
module MT19937 = struct
  ...
  type t = { st : int32 array; mutable i : int }
  ...
  let genrand mt =
    let gen k1 k2 k3 =
      ...
      let y =
        Int32.(
          Array.get mt.st k1 land upper_mask
          lor (Array.get mt.st k2 land lower_mask))
      in
      (* mt->st[k1] = mt->st[k3] ^ (y >> 1) ^ mag01[y & 0x1]; *)
      let v = Int32.(Array.get mt.st k3 lxor (y lsr 1)) in
      Array.set mt.st k1
        Int32.(v lxor Array.get mag01 (to_int_trunc (y land 1l)))
    in
